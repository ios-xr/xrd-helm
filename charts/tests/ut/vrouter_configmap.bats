...
@test "vRouter ConfigMap: Global annotations and commonAnnotations can be added and are correctly merged" {
    template \
...
@test "vRouter ConfigMap: Global labels and commonLabels can be added and are correctly merged" {
    template --set 'global.labels.foo=bar' --set 'commonLabels.baz=baa' --set 'config.script=foo'
